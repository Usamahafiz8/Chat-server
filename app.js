...
io.on("connection", (socket) => {
  ...
  socket.on(
    "getuserMessages",
    async ({ conversationId, senderemail, url, auth_token }) => {
      try {
        ...
        if (url && auth_token) {
          try {
            ...
            if (response && response.data.email) {
              ...
              socket.emit("getuserMessagesResponse", {
                error:
                  "API data is incomplete or invalid. Please provide senderemail manually.",
              });
              return;
            }
          } catch (apiError) {
            socket.emit("getuserMessagesResponse", {
              error: "API request failed. Please provide senderemail manually.",
            });
            return;
          }
        } else if (senderemail) {
          ...
          socket.emit("getuserMessagesResponse", {
            error: "Please provide senderemail",
          });
          return;
        }
        ...
        const conversation = await Conversation.findOne({
          _id: conversationId,
          useremail: userEmailFromAPI,
        }).populate("messages");
        // console.log(conversation);
        if (!conversation) {
          socket.emit("getuserMessagesResponse", {
            error: "Conversation not found or invalid senderemail",
          });
          return;
        }
        ...
        if (senderemail && conversation.useremail !== senderemail) {
          socket.emit("getuserMessagesResponse", {
            error: "Invalid senderemail for this conversation",
          });
          return;
        }
  ...
  // Assuming you have already set up your socket.io instance

  // Listen for incoming messages from the client
  socket.on(
    "userSendMessage",
    async ({ conversationId, senderemail, message }) => {
      console.log("sending ", senderemail, message);
      try {
        ...
        // Emit the new message to the appropriate socket room
...
app.post("/api/user/auth/start-conversation", async (req, res) => {
  try {
    ...
    if (!isEmpty(url) && !isEmpty(auth_token)) {
      try {
        ...
        if (response) {
          ...
          return res
            ...
            .json({
              error:
                "API data is incomplete or invalid. Please provide name, email, and role manually.",
            });
        }
      } catch (apiError) {
        ...
        return res
          ...
          .json({
            error:
              "API request failed. Please provide name, email, and role manually.",
          });
      }
      ...
      return res
        .status(400)
        .json({ error: "Please provide name, email, and role manually" });
    }
    ...
    if (!admin) {
      return res
        .status(404)
        .json({ error: "No admins available to start a conversation" });
    }
...
//       return res
//         .status(404)
//         .json({ error: "Conversation not found or invalid senderemail" });
//     }
...
app.get(
  "/api/admin/conversations/:adminId",
  verifyAdminToken,
  async (req, res) => {
    try {
...
app.get(
  "/api/admin/conversation/messages/:conversationId",
  verifyAdminToken,
  async (req, res) => {
    try {
      const conversationId = req.params.conversationId;
...
app.post(
  "/api/admin/conversation/send-message/:conversationId",
  async (req, res) => {
    try {
